...
AddEventHandler('onClientResourceStart', function(resourceName)
    ...
    AddTextEntry('TJ_NO_ROUTES_AVAILABLE', 'All delivery routes are currently occupied. Please wait a few minutes for a route to clear and try again.')
    AddTextEntry('TJ_NO_TRK_SPAWN', 'No truck spawn available wait a couple minutes and retry. Alternatively contact staff if the issue persists.')
    AddTextEntry('TJ_NO_TRL_SPAWN', 'No trailer spawn available wait a couple minutes and retry. Alternatively contact staff if the issue persists.')
    ...
    AddTextEntry('TJ_RETURN_TO_DEPOT', 'Return to the depot ~HUD_COLOUR_ORANGEDARK~~BLIP_CAPTURE_THE_FLAG~~s~ to receive your payment.')
    ...
    AddTextEntry('TJ_PAYMENT_RECEIVE', 'Great Work! For finishing ~1~ deliveries you have been paid ~HUD_COLOUR_GREEN~~1~~s~.')
    ...
    AddTextEntry('TJ_TRAILER_DISCONNECTED_FAR', 'Trailer disconnected! Recover the trailer to complete your job and get paid!')
    AddTextEntry('TJ_TRAILER_DISCONNECTED_CLOSE', 'Trailer disconnected! Disconnect the trailer when the marker turns green.')

    AddTextEntry('TJ_ROUTE_NOT_COMPLETE', 'You can\'t continue your shift because your previous route has not been completed')
